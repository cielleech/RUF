...
local checkers_Spell = setmetatable({}, {
  __index = function(t, spellIdx)
    local func = function(unit)
      if C_Spell.IsSpellInRange(spellIdx, BOOKTYPE_SPELL, unit) == 1 then
         return true
      end
    end
    t[spellIdx] = func
...
local function getNumSpells()
  local skillLineInfo = GetSpellTabInfo(GetNumSpellTabs())
...
local function findSpellIdx(spellName)
  ...
  for i = 1, getNumSpells() do
    local spell, rank = GetSpellBookItemName(i, 0)
